-- Schema without vector extension (for initial setup)
-- Use this if pgvector is not installed yet
-- After installing pgvector, run: CREATE EXTENSION IF NOT EXISTS "vector";
...
-- Storage 2: Recent Chapters (without vector for now)
CREATE TABLE IF NOT EXISTS recent_chapters (
  ...
  embedding_vector BYTEA, -- Temporary: use BYTEA instead of vector(384)
...
-- Chunk-level embeddings (without vector for now)
CREATE TABLE IF NOT EXISTS chapter_chunks (
  ...
  chunk_embedding BYTEA, -- Temporary: use BYTEA instead of vector(384)
  word_count INTEGER,
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
-- Utility Tables (without vector for now)
CREATE TABLE IF NOT EXISTS embedding_cache (
  content_hash TEXT PRIMARY KEY,
  embedding_vector BYTEA NOT NULL, -- Temporary: use BYTEA instead of vector(384)
...
-- Archive Tables (without vector for now)
